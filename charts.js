...
function buildCharts(patientID) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var filteredSample = samples.filter(sampleName => sampleName.id ==patientID)[0]
    // Deliverable 3: 1. Create a variable that filters the metadata array for the object with the desired sample number.
    var filteredMetaData = metadata.filter(sampleName => sampleName.id ==patientID)[0]
    ...
    var otu_ids = filteredSample.otu_id
    var otu_labels = filteredSample.otu_labels
    var sample_values = filteredSample.sample_values
    // Deliverable 3: 3. Create a variable that holds the washing frequency.
    var washFreq = parseInt(filteredMetaData.wfreq)
